...
PRIMARY_FONT = NOTO_FONT_DIR

# --- Extended Noto Font URLs (Full Unicode Coverage) ---
NOTO_FONTS = {
    ...
    "NotoSansThai-Regular.ttf": "https://github.com/google/fonts/raw/main/ofl/notosansthai/NotoSansThai-Regular.ttf",
    ...
    "NotoSansHebrew-Regular.ttf": "https://github.com/google/fonts/raw/main/ofl/notosanshebrew/NotoSansHebrew-Regular.ttf",
}

# --- Multilingual Font Order ---
MULTILINGUAL_FONTS = [
    PRIMARY_FONT,
    *(os.path.join(NOTO_FONT_DIR, name) for name in NOTO_FONTS.keys()),
    FALLBACK_FONT,
]
...
async def ensure_fonts():
    ...
    async def download_font(url, path):
        try:
            ...
            pass  # silent fail; fallback fonts will be used
...
def get_dominant_color_and_brightness(img: Image.Image):
    ...
    avg = (r + g + b) // 3
    color = (int((r + avg) / 2), int((g + avg) / 2), int((b + avg) / 2))
    return color, tone

# --- Ensure progress bar color has contrast ---
def adjust_color_for_contrast(color, tone):
    r, g, b = color
    brightness = 0.299*r + 0.587*g + 0.114*b
    # If too similar to background tone, shift it
    if tone == "dark" and brightness < 90:
        r, g, b = min(r + 60, 255), min(g + 60, 255), min(b + 60, 255)
    elif tone == "light" and brightness > 180:
        r, g, b = max(r - 60, 0), max(g - 60, 0), max(b - 60, 0)
    return (r, g, b)
...
def choose_title_font(text, font_path_hint, max_width, max_lines=2):
    for size in (48, 44, 40, 36, 32, 30, 28, 26, 24):
        ...
        if len(lines) <= max_lines and all(d.textlength(line, font=f) <= max_width for line in lines):
            return f
...
async def get_thumb(videoid: str) -> str:
    ...
    dom_color = adjust_color_for_contrast(dom_color, tone)
    ...
    text_x, text_max_w = 90 + 500 + 60, 640
