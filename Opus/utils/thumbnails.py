...
async def get_thumb(videoid: str, progress: float = 0.4, volume: float = 0.7) -> str | None:
    ...
    try:
        ...
        except TypeError:               # sync fallback
    ...
    smask = Image.new("L", (album_w + 40, album_h + 40), 0)
    ImageDraw.Draw(smask).rounded_rectangle(
        (20, 20, album_w + 20, album_h + 20), radius=RADIUS, fill=180
    )
    smask = smask.filter(ImageFilter.GaussianBlur(10))
    ...
    # filled part (dominant colour from cover)
    ...
    draw.rounded_rectangle(
        (bar_x0, vol_y, bar_x1, vol_y + vol_h),
        radius=2, fill=(200, 200, 200)
    )
    draw.rounded_rectangle(
        (bar_x0, vol_y, bar_x0 + vol_fill_w, vol_y + vol_h),
